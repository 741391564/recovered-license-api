...
function aesEncryptCbc(key, iv, plain) {
  ...
  return Buffer.concat([
    cipher.update(Buffer.isBuffer(plain) ? plain : Buffer.from(String(plain), "utf8")),
    cipher.final()
  ]);
}
...
function customB64Encode(buf) {
  return Buffer.from(buf).toString("base64").replace(/[A-Za-z0-9+/]/g, ch => {
    return QUEEN_CUSTOM_B64[STD_B64.indexOf(ch)];
  });
}
...
function validateKamiForQueen(body, req) {
  ...
  if (!kami) return { ok: false, code: 404, message: "卡密不存在" };

  const db = readDb();
  const kamiHash = sha256(kami);
  const key = db.keys[kamiHash];

  if (!key || !key.enabled) return { ok: false, code: 404, message: "卡密不存在" };
  if (key.expire_unix <= nowUnix()) return { ok: false, code: 403, message: "卡密已过期" };

  key.devices = key.devices || [];
  if (udid && !key.devices.includes(udid)) {
    if (key.devices.length >= Number(key.max_devices || 1)) {
      return { ok: false, code: 403, message: "设备数量超限" };
    }
...
function queenHybridResponse(api, body, session, requestBody = {}) {
  const encryptedRequest = !!(
    requestBody &&
    (requestBody.payload || requestBody.encrypted_session_key || requestBody.encryptedSessionKey)
  );

  const wantsEncrypted =
    encryptedRequest ||
    requestBody.response_format === "encrypted" ||
    requestBody.format === "encrypted";

  if (!wantsEncrypted) return body;
...
async function handleQueenApi(req, res, api) {
  ...
  console.log(
    `[queen] api=${api} keys=${Object.keys(body || {}).join(",") || "-"} legacy=false payload=${
      body && body.payload ? String(body.payload).length : 0
    }`
  );
...
async function handle(req, res) {
  ...
  console.log(
    `[${new Date().toISOString()}] ${req.method} ${url.pathname}${url.search} queenApi=${
      queenApi || "-"
  ...
  if (req.method === "POST" && url.pathname === "/admin/keys/disable") {
    ...
    if (!db.keys[hash]) {
      return json(res, 404, { success: false, message: "license not found" });
    }
  ...
  if (req.method === "POST" && (url.pathname === "/api/v1/auth/verify" || url.pathname === "/verify")) {
    ...
    if (!kami || !udid) {
      return json(res, 400, { success: false, message: "kami and udid required" });
    }
    ...
    if (!key || !key.enabled) {
      return json(res, 403, { success: false, message: "invalid license" });
    }

    if (key.expire_unix <= nowUnix()) {
      return json(res, 403, { success: false, message: "license expired" });
    }
  ...
  if (req.method === "POST" && (url.pathname === "/api/v1/auth/heartbeat" || url.pathname === "/hb")) {
    ...
    if (!state.ok) {
      return json(res, 401, { success: false, message: state.message });
    }
  ...
  if (req.method === "GET" && (url.pathname === "/api/v1/features" || url.pathname === "/feature")) {
    ...
    return json(res, 200, {
      success: true,
      features: state.key.features || DEFAULT_FEATURES
    });
  }
